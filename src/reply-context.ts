...
let activeReplySelfId: string | number | null = null;

export function setActiveReplyTarget(to: string): void {
  activeReplyTarget = to;
}

export function setActiveReplySelfId(selfId: string | number | null): void {
  activeReplySelfId = selfId;
}

export function getActiveReplySelfId(): string | number | null {
...
function normalizeTargetForCompare(t: string): string {
  const s = (t || "").replace(/^(onebot12|onebot|qq|lagrange|ob12):/i, "").trim().toLowerCase();
  return s || "";
}
 ...
 */
export function shouldBlockSendInForwardMode(type: "private" | "group", targetId: string | number): boolean {
  if (!forwardSuppressDelivery) return false;
  const selfId = activeReplySelfId;
  if (type === "private") {
    return String(targetId) !== String(selfId);
  }
  return true;
}
...
export function resolveTargetForReply(to: string): string {
  ...
  if (!m) return to;
  const groupId = m[1];
  const normalizedTo = to.replace(/^(onebot12|onebot|qq|lagrange|ob12):/i, "").trim();
